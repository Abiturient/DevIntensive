...
public class UserInfoValidator implements TextWatcher {
    ...
    private int mTimerLength = ConstantManager.ERROR_TIMER_LENGTH_NORMAL;

    public UserInfoValidator(Context context, EditText editText, TextInputLayout textInputLayout, LinearLayout linearLayout) {
    ...
    private void errorHandler(Boolean isError, String errorType) {
        ...
        if (isError) {
            ...
            lp.height = mResources.getDimensionPixelSize(R.dimen.size_big_88);
            ...
            if (!handlerIsAlive) {
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        lp.height = mResources.getDimensionPixelSize(R.dimen.size_big_72);
                        mLinearLayout.setLayoutParams(lp);
    ...
    private String isValidPhone(String phone) {
        ...
        if (checkPhone.matches("(\\d*\\D\\d*)*")) {
            mTimerLength = ConstantManager.ERROR_TIMER_LENGTH_LONG;
        ...
        if (checkPhone.length() < MAX_DIGITS_COUNT) {
            ...
            if (checkPhone.length() > MAX_DIGITS_COUNT) {
                mTimerLength = ConstantManager.ERROR_TIMER_LENGTH_SHORT;
                return mResources.getString(R.string.error_editText_phone_wrong_length);
            }
        }
        return null;
